...
def get_model_field(point_cloud, probe_points, is_training, bn_decay=None):
    ...
    net2 = tf_util.fully_connected(tf.concat([net, net1], axis=1), 32 * 64, activation_fn=None, scope='fc4')
    net2 = tf.reshape(net2, [batch_size, 32, 64])
    net3 = tf_util.fully_connected(tf.concat([net, net2], axis=1), 64 * 32, activation_fn=None, scope='fc5')
    net3 = tf.reshape(net3, [batch_size, 64, 32])
    net4 = tf_util.fully_connected(tf.concat([net, net3], axis=1), 32 * 1, activation_fn=None, scope='fc6')
    net4 = tf.reshape(net4, [batch_size, 32, 1])
