...
def conditional_generator(inputs):
    ...
    noise2 = tf.random_normal([128, 512])
    cloud = generate_cloud(tf.expand_dims(feature, axis=1), tf.expand_dims(noise, axis=1))
...
#tfgan.gan_train(train_ops, hooks=[tf.train.StopAtStepHook(num_steps=FLAGS.max_number_of_steps)],logdir='./log/gan_log/')
...
saver = tf.train.import_meta_graph('./log/gan_log/model.ckpt-862.meta')
saver.restore(sess, './log/gan_log/model.ckpt_862.data-00000-of-00002')
#generated_demos = sess.run(gan_model.generated_data)

h5r = h5py.File('demo.h5', 'w')
